# Update luxury styling Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Update final overlay heading color to white and make it render on two lines on both mobile and desktop.

**Architecture:** Edit `src/components/Hero.tsx` to update heading text classes and contents.
...
- [ ] **Step 1: Replace final overlay JSX in Hero.tsx**
  * Modify `src/components/Hero.tsx` to update the final overlay block to use `text-white` and a single two-line block:
    ```tsx
